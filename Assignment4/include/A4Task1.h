...
public:
    ...
    A4Task1Render(AppResources &app, Render &render): app(app), render(render) {};
    void prepare(A4Task1Data &data) {
        { // Opaque
            vk::ShaderModule vertexM, fragmentM;
            // Create Shader Modules
            Cmn::createShader(app.device, vertexM, "./shaders/soup.vert.spv");
            Cmn::createShader(app.device, fragmentM, "./shaders/phong.frag.spv");
            ...
            //( x start coordinate,y start coordinate, width of viewport, height of viewport
			// min framebuffer depth, max framebuffer depth)
        ...
        { // Transparent
            ...
            Cmn::createShader(app.device, vertexM, "./shaders/particle.vert.spv");
            ...
            vk::PipelineInputAssemblyStateCreateInfo inputAssemblySCI = {
                {}, 
                vk::PrimitiveTopology::ePointList,      // Primitive type to assemble vertices as
            ...
            vk::PipelineMultisampleStateCreateInfo multisampleSCI = {
                {}, 
                vk::SampleCountFlagBits::e1,      // Numbe rof samples to use per fragment
                false,                            // Enable multisample shading or not
                0.f, 
                nullptr,
                false,
            ...
            vk::GraphicsPipelineCreateInfo cI = {
                ...
                data.layout,                     // Pipeline Layout pipeline should use                    
                render.renderPass,               // Render pass description the pipeline is compatible with
                1,                               // Subpass of render pass to use with pipeline
                // Pipeline Derivatives : Can create multiple pipelines that derive from one another for optimisation
                {},                              //basePipelineHandle - Existing pipeline to derive from...
                0                                //basePipelineIndex - or index of pipeline being created to derive from (in case creating multiple at once)
            };
            ...
            if (pipelines.result != vk::Result::eSuccess)
                throw std::runtime_error("Pipeline creation failed");
            transparentPipeline = pipelines.value[0];
        ...
        { // Lines
            vk::ShaderModule vertexM, fragmentM;
            Cmn::createShader(app.device, vertexM, "./shaders/lines.vert.spv");
            Cmn::createShader(app.device, fragmentM, "./shaders/white.frag.spv");
            ...
            vk::PipelineRasterizationStateCreateInfo rasterizationSCI = {
                ...
                false,                                // Change if fragments beyond near/far planes are clipped (default) or clamped to plane
                false,                                // Whether to discard data and skip rasterizer. Never creates fragments, only suitable for pipeline without framebuffer output
                vk::PolygonMode::eLine,               // How to handle filling points between vertices
                vk::CullModeFlagBits::eNone,          // Which face of a tri to cull
                vk::FrontFace::eCounterClockwise,     // Winding to determine which side is front
                false,                                // Whether to add depth bias to fragments (good for stopping "shadow acne" in shadow mapping)
                0.f,
            ...
            if (pipelines.result != vk::Result::eSuccess)
                throw std::runtime_error("Pipeline creation failed");
            linesPipeline = pipelines.value[0];
    ...
    void renderFrame(A4Task1Data &data) {
        ...
        [&](vk::CommandBuffer &cb) {
            cb.bindPipeline(
                vk::PipelineBindPoint::eGraphics, //VK pipeline bind point graphics
                opaquePipeline                    
            );
            A4Task1Data::PushConstant pC;
            pC.mvp = render.camera.viewProjectionMatrix();
            pC.pos = glm::vec4(render.camera.position, 0.f);
            cb.pushConstants(data.layout, vk::ShaderStageFlagBits::eAll, 0, sizeof(pC), &pC);
            cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, data.layout, 0, 1, &data.descriptorSet, 0, nullptr);
...
class A4Task1 {
    AppResources &app;  
    A4Task1Render render;

public:
    A4Task1Data data;
    
    A4Task1(AppResources &app, Render &render, const uint32_t particleCount=20, std::string objName="../Assets/cubeJump.obj") : app(app), render(app, render)
    {
        ...
        data.gAlive = makeDLocalBuffer(BFlag::eTransferDst | BFlag::eStorageBuffer, data.particleCount * 2 * sizeof(uint32_t), "gAlive");
        data.gPosLife = makeDLocalBuffer(BFlag::eTransferDst | BFlag::eStorageBuffer, data.particleCount * 2 * sizeof(float)*4, "gPosLife");
        data.gVelMass = makeDLocalBuffer(BFlag::eTransferDst | BFlag::eStorageBuffer, data.particleCount * 2 * sizeof(float)*4, "gVelMass");
        data.gTriangleSoup = makeDLocalBuffer(BFlag::eTransferDst | BFlag::eStorageBuffer, tSoup.size() * sizeof(float) * 4, "gTriangleSoup");
        data.gNormals = makeDLocalBuffer(BFlag::eTransferDst | BFlag::eStorageBuffer, normals.size() * sizeof(float) * 4, "gNormals");
        data.gForceLines = makeDLocalBuffer(BFlag::eTransferDst | BFlag::eStorageBuffer, pForce.size() * sizeof(float) * 4, "gForceLines");
        ...
        fillDeviceWithStagingBuffer(app.pDevice, app.device, app.transferCommandPool,app.transferQueue, data.gForceLines, pForce);
        ...
        vk::PushConstantRange pcr(vk::ShaderStageFlagBits::eAll, 0, sizeof(A4Task1Data::PushConstant));
    ...
    void loop(A4Task1Solution &solution);
    void prepare(A4Task1Solution &solution);
...
private:
    ...
    */
    void make3DTexture()
    {
        ...
        transitionImageLayout(
            ...
            data.textureImage, vk::Format::eR32G32B32A32Sfloat, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal);
        copyBufferToImage(app.device, app.transferCommandPool, app.transferQueue, staging.buf, data.textureImage, data.V_RESx, data.V_RESy, data.V_RESz);
        transitionImageLayout(
            app.device, app.computeCommandPool, app.computeQueue,
            data.textureImage, vk::Format::eR32G32B32A32Sfloat, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);
	...
	{
		pForce[ 2 * i ]={float(rand()) / float(RAND_MAX),
    ...
    */
    void initParticles(){
        ...
        fillDeviceWithStagingBuffer(app.pDevice, app.device, app.transferCommandPool,app.transferQueue, data.gVelMass, pVelMass);
        fillDeviceWithStagingBuffer(app.pDevice, app.device, app.transferCommandPool,app.transferQueue, data.gPosLife, pPosLife);
        fillDeviceWithStagingBuffer(app.pDevice, app.device, app.transferCommandPool,app.transferQueue, data.gAlive, pAlive);
    }
