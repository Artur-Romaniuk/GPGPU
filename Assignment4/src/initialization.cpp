...
void initApp(AppResources &app, bool withWindow)
{
    app.window = nullptr;
    if (withWindow) {
        if (!glfwInit())
        ...
        if (!glfwVulkanSupported())
            throw std::runtime_error("GLFW reports to have no Vulkan support! Maybe it couldn't find the Vulkan loader!");
        glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
        glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);
        app.window = glfwCreateWindow(2500, 1500, "GPGPU", nullptr, nullptr);
    }
...
std::tuple<uint32_t, uint32_t, uint32_t> getGCTQueues(vk::PhysicalDevice &pDevice)
{
    ...
    for (uint32_t j = 0; j < queueFamilyProperties2.size(); j++)
    {
        ...
        if (properties.queueFlags & QFB::eTransfer)
            if (otq == -1 && !(properties.queueFlags & QFB::eCompute || properties.queueFlags & QFB::eGraphics)) otq = j;
    }
...
void createSwapchain(AppResources &app)
{
    ...
    vk::SwapchainCreateInfoKHR createInfo(
        ...
        vk::CompositeAlphaFlagBitsKHR::eOpaque,               // How to handle blending images with external graphics (e.g. other windows)
