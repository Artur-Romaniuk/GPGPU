    ...
    app(app), data(datad), workGroupSize_x(workGroupSize_x), triangleCacheSize(triangleCacheSize)
{
    ...
    std::string compute = "./shaders/particles.comp.spv";
...
void ParticlesA4Task1Solution::prepare(){}

void ParticlesA4Task1Solution::compute()
{
    ...
    cb.pushConstants(data.layout, vk::ShaderStageFlagBits::eAll,
                     0, sizeof(A4Task1Data::PushConstant), &data.push);
    cb.dispatch(dx, 1, 1);
...
std::vector<float> ParticlesA4Task1Solution::result() const
{
...
void ParticlesA4Task1Solution::cleanup()
{
